...
- Because the enterprise API can read all system shipments, `AUTH_REQUIRED=true` is mandatory in central-API mode. Store the enterprise Key only in the server-side secret manager; do not expose it to the browser, exports, logs, or support chat. `ALLOW_UNAUTHENTICATED_INTERNAL_QUERY=true` exists only for temporary loopback demos and must never be used in a deployed service.
